...
class Checkout extends Component {
    ...
    editCheckoutHandler = (event) => {
        ...
        axios.get(`/fooder_checkout/${_checkoutEditButtonId}`)
            .then(res =>{
                ...
                this.setState({display_edit: true, display_edit_id: _checkoutEditButtonId});
            });
    }
    ...
    deleteCheckoutAddOnHandler = (event) => {
        event.preventDefault();
        const _checkoutDeleteButtonAddOn = event.currentTarget.value
        let _checkOutAddOn = [...this.state.foodercheckout_addon,_checkoutDeleteButtonAddOn]
        let deleteCheckout = _checkOutAddOn.filter(fdelete => fdelete !== _checkoutDeleteButtonAddOn);
        this.setState({foodercheckout_addon:deleteCheckout})
    }
                    ...
                    <div className={classes.CheckoutBlockSelector}>
                        ...
                        {this.state.foodercheckout.map((fcheckout) =>
                            <div key={fcheckout._id}>
                                ...
                                <div style={{marginLeft:"15px"}}>
                                    {
                                        fcheckout.addon.map(fadd => 
                                            <div className={classes.checkoutEditColumn}>
                                                <span key={fadd} className={classes.checkoutaddOn} >
                                                    <button id={fcheckout._id} value={fadd}><FaTimes size={20}/></button>
                                                    {fadd}
                                                </span>
                                            </div>)
                                    }
                                ...
                                <button value={fcheckout._id} type="submit" className={classes.CheckoutEditButton} onClick={this.editCheckoutHandler}><BsListTask size={32}/></button>
                                <button value={fcheckout._id} type="submit" className={classes.CheckoutDeleteButton} onClick={this.deleteCheckoutHandler}><FaTimes size={32}/></button>
